...
__global__ void contains_clustered_kernel(const filter_t filter,
                                          ...
                                          u32 *__restrict__ keys, u32 *__restrict__ hash_keys, u32 key_cnt,
                                          $u32 *__restrict__ result_bitmap, $u32 *__restrict__ positions) {
	// who am I?
...
//===----------------------------------------------------------------------===//
template <typename filter_t>
__global__ void contains_shared(const filter_t filter, const typename filter_t::word_t *__restrict__ word_array,
                                u32 *__restrict__ keys, u32 key_cnt, $u32 *__restrict__ result_bitmap) {
	// who am I?
	u32 wid = global_warp_id();
	u32 lid = warp_local_thread_id();
	...
	for ($u32 i = 0; i != elements_per_thread; i++) {
	   ...
	   const auto block_ptr = &word_array[bitvector_word_idx];*/
		auto is_contained = (read_pos < key_cnt) ? filter.contains_sorted(word_array, keys[read_pos]) : false;
		u32 bitmap = __ballot_sync(0xffffffff, is_contained);
		thread_local_bitmap = (lid == i) ? bitmap : thread_local_bitmap;
		read_pos += warp_size;
	}
