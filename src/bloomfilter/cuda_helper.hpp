...
size_t get_temp_storage_requirement(size_t data_length)
{
    size_t temp_storage_requirement { 0 };
    auto status = cub::DeviceRadixSort::SortPairs(
        nullptr, temp_storage_requirement, 
        static_cast<Key*  >(nullptr), 
        static_cast<Key*  >(nullptr), 
        static_cast<Value*>(nullptr), 
        static_cast<Value*>(nullptr), 
        data_length, 24, 32);
    cuda::throw_if_error(status, "CUB could not determine the temporary storage size requirement for sorting key-value pairs");
...
size_t get_temp_storage_requirement(size_t data_length) {
    ...
    if(status != cudaSuccess) 
        std::cerr << "CUB could not determine the temporary storage size requirement for sorting key-value pairs" << std::endl;
    return temp_storage_requirement;
}
//===----------------------------------------------------------------------===//

template <typename Key, typename Value> 
void sort_with_cub(size_t data_length, Key host_keys, Value host_values){
    ...
    auto status = cub::DeviceRadixSort::SortPairs(
            temp_storage.get(), temp_storage_size, 
            device_keys.get(), sorted_device_keys.get(),
            device_values.get(), sorted_device_values.get(),
            data_length);
