...
template<typename T, u32 items_per_thread,
...
contains_kernel(
  ...
  for ($u32 i = 0; i != elements_per_thread; ++i) {
    auto is_contained = (read_pos < key_cnt)
        ? filter.contains(word_array, keys[read_pos])
        : false;
...
struct cuda_filter {
  ...
  void
  contains(u32* __restrict keys, u32 key_cnt, $u64* __restrict bitmap) {
  ...
  contains(u32* __restrict keys, u32 key_cnt, $u64* __restrict bitmap,
    ...
    if (cu_err != cudaSuccess) std::cerr << "allocation failed" << std::endl;
    cuda_check_error();
    cu_err = cudaMemcpy(device_keys, keys, device_keys_size,
...
//      contains_kernel<<<block_cnt, block_size>>>(
      contains_kernel_with_block_prefetch<<<block_cnt, block_size>>>(
          filter, device_word_array, device_keys, key_cnt, device_bitmap);
      cu_err = cudaDeviceSynchronize();
      cuda_check_error();
    }
...
//      contains_kernel<<<block_cnt, block_size>>>(
    ...
    u64 probes_per_second =
        static_cast<u64>(key_cnt * repeats / duration.count());

    perf_data.probes_per_second = probes_per_second;

    // Copy back the resulting bitmap to the host memory.
    cu_err = cudaMemcpy(bitmap, device_bitmap, device_bitmap_size,
  ...
  struct probe {
    ...
    /// C'tor.
    probe(const cuda_filter& cuda_filter_instance,
        u64 batch_size,
        const cudaStream_t& cuda_stream)
    ...
    probe(probe&& src) noexcept
      : cuda_filter_instance(src.cuda_filter_instance),
        cuda_stream(src.cuda_stream),
        start_event(nullptr),
        stop_event(nullptr),
    ...
    contains(const key_t* keys, const std::size_t key_cnt) {
      if (key_cnt == 0) return;
      ...
      cuda_check_error();
      cudaMemcpyAsync(device_keys, keys, key_cnt * sizeof(key_t),
...
//      perf_data.cuda_block_cnt = block_cnt;
      ...
      contains_kernel_with_block_prefetch<<<block_cnt, block_size, 0, cuda_stream>>>(
          cuda_filter_instance.filter,
          cuda_filter_instance.device_word_array,
          device_keys,
          key_cnt,
          device_bitmap);
      // Copy back the result bitmap to pre-allocated host memory.
      cudaMemcpyAsync(host_bitmap, device_bitmap, bitmap_size_bytes,
    ...
    struct result_view {
      ...
      u1 __forceinline__
      is_match(std::size_t key_idx) const noexcept {
