...
struct bbf_block_switch
{
    ...
    using sectorized_rnd = dtl::multiword_block<key_t, word_t, word_cnt,
    ...
    using zoned = dtl::multizone_block<filter_key_t, word_t, word_cnt, zone_cnt,
                                       k, dtl::hasher, hash_value_t, block_hash_fn_idx, 0, zone_cnt, early_out>;

    /// Refers to the implementation.
    using type =
        typename std::conditional<(zone_cnt > 1),
                                  zoned,
...
/// LOAD-CMP sequences.
static constexpr std::size_t get_word_access_cnt(u32 word_cnt, u32 sector_cnt, u32 zone_cnt,
                                                 u32 k)
{
    return (zone_cnt > 1)
               ? zone_cnt
