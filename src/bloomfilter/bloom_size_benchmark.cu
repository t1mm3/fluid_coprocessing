...
void benchmark(const std::size_t m,
    ...
    for (std::size_t i = 0; i < to_insert.size(); ++i) {
        ...
        if (!filter.contains_with_hash(&filter_data[0], hash_value)) {
    ...
    for(std::size_t i = 0; i != to_lookup.size(); ++i) {
        ...
        if(filter.contains_with_hash(&filter_data[0], hash_val)) {
            matches++;
        }
    }
    // // cpu BF
...
int main(int argc, char** argv) {
    ...
    std::size_t insert_cnt = 1<<28; //10M
    ...
    for(size_t i = 1; i != to_lookup.size(); ++i){
        to_insert[i] = to_lookup[i];
    }
