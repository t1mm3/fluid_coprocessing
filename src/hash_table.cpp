...
void HashTablinho::_Insert(bucket_t* CPU_R buckets, bucket_t* CPU_R heads,
uint32_t* CPU_R hash, uint32_t mod_mask, bucket_t* CPU_R next_vector, size_t
next_stride, int* CPU_R sel, int num) { #if 1 Vectorized::map(sel, num, [&]
(auto i) { bucket_t bucket = buckets[i]; size_t slot = hash[i] & mod_mask;
                bucket_t head;
                {
                        head = heads[slot];
                        next_vector[bucket * next_stride] = head;
                } while (__sync_bool_compare_and_swap(&heads[slot], head,
bucket));
        });
#else
...
void HashTablinho::BucketInit(bucket_t* CPU_R buckets, bucket_t* CPU_R heads,
uint32_t* CPU_R hash, uint32_t mod_mask, int* CPU_R sel, int num) {
        Vectorized::map(sel, num, [&] (auto i) {
                size_t slot = hash[i] & mod_mask;
...
void HashTablinho::FinalizeBuild() {
        ...
        while (i < (size_t)num_buckets) {
                ...
                Vectorized::read(hashs, hash_vector, i, hash_stride, nullptr,
num);
                ...
                _Insert(tmp_buckets, heads, hashs, mod_mask, next_vector,
next_stride, nullptr, num);

                i+=num;
        }
}

void HashTablinho::Probe(ProbeContext &ctx, bool* match, int32_t* keys,
uint32_t* hash, int* in_sel, int in_num) { assert(heads);
        ...
        while (n) {
                // check keys
                Vectorized::check<int32_t>(match, keys, key_vector, tmp_buckets,
key_stride, sel, n);

                n = Vectorized::select_not_match(tmp_sel, match, sel, n);

                Vectorized::gather_next(tmp_buckets, next_vector, tmp_buckets,
next_stride, sel, n);

                Vectorized::map_not_match_bucket_t(match, tmp_buckets, 0, sel,
n); n = Vectorized::select_match(tmp_sel, match, sel, n);
        };
}

void HashTablinho::ProbeGather(ProbeContext &ctx, int32_t* coldata, int64_t
colidx, int* sel, int n) {
        // Vectorized::gather_ptr<int32_t>(coldata, (int32_t**)ctx.tmp_buckets,
colidx, sel, n);
}*/
