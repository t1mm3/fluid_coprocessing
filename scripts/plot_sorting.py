...
def plot_sorting_time():
    df = pd.read_csv("{}/bench_bits.csv".format(result_path), sep=';', usecols=['Bloom filter size (MiB)','bits to sort','Sort time (ms)', 'Probe time (ms)'])
    print(df)
    bf16 = df[df['Bloom filter size (MiB)']==16]
    #bf32 = df[df['Bloom filter size (MiB)']==32]
    bf64 = df[df['Bloom filter size (MiB)']==64]
   # bf128 = df[df['Bloom filter size (MiB)']==128]
    ...
    ax1.loglog(bf16['bits to sort'] , bf16['Probe time (ms)'] / tp_div, linestyle='--', marker='o', color=colors[0], label="BF Size 16MiB", basex=2)
    #ax1.loglog(bf32['bits to sort'] , bf32['Total throughput'] / tp_div, linestyle='--', marker='o', color=colors[1], label="BF Size 32MiB", basex=2)
    ax1.loglog(bf64['bits to sort'] , bf64['Probe time (ms)'] / tp_div, linestyle='--', marker='o', color=colors[1], label="BF Size 64MiB", basex=2)
    #ax1.loglog(bf128['bits to sort'] , bf128['Total throughput'] / tp_div, linestyle='--', marker='o', color=colors[3], label="BF Size 128MiB", basex=2)
    #ax1.loglog(bf256['bits to sort'] , bf256['Total throughput'] / tp_div, linestyle='--', marker='o', color=colors[4], label="BF Size 256MiB", basex=2)
    ax1.loglog(bf512['bits to sort'] , bf512['Probe time (ms)'] / tp_div, linestyle='--', marker='o', color=colors[2], label="BF Size 512MiB", basex=2)
    ax1.loglog(sort['bits to sort'] , sort['Sort time (ms)'] / tp_div, linestyle='--', marker='o', color=colors[3], label="Sorting", basex=2)
