...
struct cuda_filter {
  ...
  void contains_clustering(u32 *__restrict__ keys, u32 key_cnt, $u32 *__restrict__ bitmap,
    ...
    calculate_hash_kernel<<<block_cnt, block_size>>>(filter, d_keys, key_cnt,
													 d_keys_hash, d_positions);
	...
	perf_data.hash_throughput =
		static_cast<u64>(key_cnt * repeats / duration_hash.count());
	...
	auto temp_storage_size =
		get_temp_storage_requirement<key_t, key_t>(key_cnt);
	unsigned char *temp_storage;
	cudaMalloc((void **)&temp_storage, temp_storage_size);
	auto status = cub::DeviceRadixSort::SortPairs(
		temp_storage, temp_storage_size, d_keys_hash, d_sorted_keys,
		d_positions, d_sorted_positions, key_cnt, BEGIN_BIT, END_BIT);
	cudaDeviceSynchronize();

	auto end_sort = std::chrono::high_resolution_clock::now();
	std::chrono::duration<double> duration_sort = end_sort - start_sort;
	perf_data.sort_time = duration_sort.count();
	perf_data.sort_throughput =
		static_cast<u64>(key_cnt * repeats / duration_sort.count());
	...
	/* if (repeats > 0) {
																	 // warm up
																	 contains_kernel
	 <<<block_count, block_size>>>( filter, device_word_array, sorted_keys_hash,
	 key_cnt, device_bitmap); cudaDeviceSynchronize(); cuda_check_error();
	 }*/
	...
	u64 probes_per_second =
		static_cast<u64>(key_cnt * repeats / duration_probe.count());

	perf_data.probes_per_second = probes_per_second;

	double total_time = static_cast<double>(
		perf_data.hash_time + perf_data.sort_time + perf_data.probe_time);
	perf_data.total_throughput =
		static_cast<u64>((key_cnt * repeats) / total_time);
